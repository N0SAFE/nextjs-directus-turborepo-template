  ...
  return (
    <div className="flex gap-4 m-4">
      <div className="flex justify-between gap-2">
        ...
        <div className="text-sm text-red-500">{issue.received.toString()}</div>
      </div>
      <div className="flex justify-between gap-2">
        <div className="text-sm text-gray-500">Expected Type: </div>
        <div className="text-sm text-red-500">{issue.expected.toString()}</div>
      </div>{" "}
    </div>
  );
}
  ...
  if (parsedEnv.success) {
    return (
      <div className="flex flex-col justify-center items-center mt-12">
        ...
        <RedirectAfterDelay delay={3000} to={searchParams.redirect || "/"} />
      </div>
    );
  }
    ...
    <div className="flex flex-col justify-center items-center mt-12">
      {issues.map((issue, index) => (
        <Card key={index} className="w-[380px]">
          <CardHeader className="text-center">
            <CardTitle className="text-red-500">{issue.message}</CardTitle>
            <CardDescription>{issue.path.join(".")}</CardDescription>
          </CardHeader>
          <CardContent>
            {renderByCode(issue)}
            <div className="flex items-center justify-between pt-8">
              <div className="text-sm text-gray-500">Code: {issue.code}</div>
