...
import { Map, Globe, Search, ExternalLink, CheckCircle, XCircle } from 'lucide-react'
import { useState, useEffect } from 'react'
import { useDevToolAPI } from '../../../hooks/useDevToolAPI'

/**
 * Routes overview component - displays application routing information from API
 */
    ...
    const loadRoutes = async () => {
      try {
        ...
        const [routesData, statsData] = await Promise.all([
          api.devtools.routes.getRoutes(),
          api.devtools.routes.getRouteStats()
        ])
...
/**
 * API Routes component - displays and tests API endpoints
 */
export function ApiRoutesComponent({ context }: { context: PluginContext }) {
  ...
  useEffect(() => {
    const loadApiRoutes = async () => {
      try {
        setLoading(true)
        const routes = await api.devtools.routes.getRoutes()
  ...
  const testApiEndpoints = async () => {
    try {
      setTesting(true)
      const results = await api.devtools.routes.testApiEndpoints()
