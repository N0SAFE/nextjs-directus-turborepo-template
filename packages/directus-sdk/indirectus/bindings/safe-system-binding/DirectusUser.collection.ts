...
export class DirectusMe extends ChainableBinding {
  ...
  >(query?: Query): Promise<ToSafeOutput<Output>> {
    return toSafe(this.request(readMeItem(query))) as unknown as Promise<
      ToSafeOutput<Output>
    >;
  }
  ...
  async readRoles<
    ...
    >,
  >(query?: Query): Promise<ToSafeOutput<Output>> {
    return toSafe(this.request(readMeRoleItem(query))) as unknown as Promise<
      ToSafeOutput<Output>
    >;
  }
  ...
  >(
    ...
    return toSafe(
      this.request(updateMeItem(patch, query)),
...
export class DirectusUserItems extends ChainableBinding {
  ...
  >(query?: Query): Promise<ToSafeOutput<Output>> {
    return toSafe(
      this.request(readDirectusUserItems(query)) as unknown as Promise<Output>,
    );
  }
  ...
  async remove<Output = void>(
    ...
    return toSafe(
      this.request(deleteDirectusUserItems(keys)) as unknown as Promise<Output>,
    );
  }
  ...
  >(options: Options): Promise<ToSafeOutput<Output>> {
    return toSafe(
      this.request(aggregateDirectusUserItems<Options>(options)).then(
        (a) => a?.[0],
      ) as unknown as Promise<Output>,
    );
  }
...
export class DirectusUserItem extends ChainableBinding {
  ...
  async remove<Output = void>(
    ...
    return toSafe(
      this.request(deleteDirectusUserItem(key)) as unknown as Promise<Output>,
    );
  }
}
