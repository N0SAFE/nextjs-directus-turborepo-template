...
export class DirectusShareItems extends ChainableBinding {
  ...
  >(query?: Query): Promise<ToSafeOutput<Output>> {
    return toSafe(
      this.request(readDirectusShareItems(query)) as unknown as Promise<Output>,
    );
  }
  ...
  >(options: Options): Promise<ToSafeOutput<Output>> {
    return toSafe(
      this.request(aggregateDirectusShareItems<Options>(options)).then(
        (a) => a?.[0],
      ) as unknown as Promise<Output>,
    );
  }
...
export class DirectusShareItem extends ChainableBinding {
  ...
  async remove<Output = void>(
    ...
    return toSafe(
      this.request(deleteDirectusShareItem(key)) as unknown as Promise<Output>,
    );
  }
}
