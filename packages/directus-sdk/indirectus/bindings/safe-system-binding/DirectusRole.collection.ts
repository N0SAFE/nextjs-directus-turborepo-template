...
export class DirectusRoleItems extends ChainableBinding {
  ...
  >(query?: Query): Promise<ToSafeOutput<Output>> {
    return toSafe(
      this.request(readDirectusRoleItems(query)) as unknown as Promise<Output>,
    );
  }
  ...
  async remove<Output = void>(
    ...
    return toSafe(
      this.request(deleteDirectusRoleItems(keys)) as unknown as Promise<Output>,
    );
  }
  ...
  >(options: Options): Promise<ToSafeOutput<Output>> {
    return toSafe(
      this.request(aggregateDirectusRoleItems<Options>(options)).then(
        (a) => a?.[0],
      ) as unknown as Promise<Output>,
    );
  }
...
export class DirectusRoleItem extends ChainableBinding {
  ...
  async remove<Output = void>(
    ...
    return toSafe(
      this.request(deleteDirectusRoleItem(key)) as unknown as Promise<Output>,
    );
  }
}
