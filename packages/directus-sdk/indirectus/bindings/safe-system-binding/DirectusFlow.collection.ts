...
export class DirectusFlowItems extends ChainableBinding {
  ...
  >(query?: Query): Promise<ToSafeOutput<Output>> {
    return toSafe(
      this.request(readDirectusFlowItems(query)) as unknown as Promise<Output>,
    );
  }
  ...
  async remove<Output = void>(
    ...
    return toSafe(
      this.request(deleteDirectusFlowItems(keys)) as unknown as Promise<Output>,
    );
  }
...
export class DirectusFlowItem extends ChainableBinding {
  ...
  async remove<Output = void>(
    ...
    return toSafe(
      this.request(deleteDirectusFlowItem(key)) as unknown as Promise<Output>,
    );
  }
}
