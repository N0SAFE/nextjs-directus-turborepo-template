...
import type { TemplateField, GroupConfiguration, IConfigService } from '../../src/types/index.js';
...
describe('GroupingService', () => {
  ...
  describe('extractGroupConfiguration', () => {
    it('should extract configuration from SYSTEM_ENV_TEMPLATE_CONFIG field', () => {
      ...
      const result = groupingService.extractGroupConfiguration(fieldsWithConfig);
  ...
  describe('generateGroupTitle', () => {
    it('should handle special cases', () => {
      expect(groupingService.generateGroupTitle('NEXT_PUBLIC')).toBe('Next.js Public Configuration');
      expect(groupingService.generateGroupTitle('DATABASE')).toBe('Database Configuration');
      expect(groupingService.generateGroupTitle('API')).toBe('API Configuration');
      expect(groupingService.generateGroupTitle('AUTH')).toBe('Authentication Configuration');
    });

    it('should generate title from key for custom groups', () => {
      expect(groupingService.generateGroupTitle('CUSTOM_SERVICE')).toBe('Custom Service Configuration');
      expect(groupingService.generateGroupTitle('PAYMENT')).toBe('Payment Configuration');
    });

    it('should handle single word keys', () => {
      expect(groupingService.generateGroupTitle('REDIS')).toBe('Redis Configuration');
    });
  });
  ...
  describe('validateGroupConfiguration', () => {
    ...
    it('should warn about empty ungrouped title', () => {
      ...
      expect(result.valid).toBe(true);
      expect(result.warnings).toContain('Ungrouped title is empty, using default');
    });
  });
