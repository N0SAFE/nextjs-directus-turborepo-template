...
describe('PromptService', () => {
  ...
  beforeEach(() => {
    mockValidationService = {
      ...
      validateField: vi.fn().mockResolvedValue({ valid: true, errors: [], warnings: [] }),
      validateUrl: vi.fn(),
  ...
  describe('promptForField', () => {
    ...
    it('should skip field when existing value found and skipExisting is true', async () => {
      ...
      const result = await promptService.promptForField(sampleField, skipContext);
    ...
    it('should use default value in non-interactive mode', async () => {
      ...
      const result = await promptService.promptForField(sampleField, nonInteractiveContext);
    ...
    it('should prompt user in interactive mode', async () => {
      ...
      expect(mockValidationService.validateField).toHaveBeenCalledWith('user-input', sampleField);
    });
    ...
    it('should handle validation failure and retry', async () => {
      ...
      vi.mocked(mockValidationService.validateField)
        .mockResolvedValueOnce({ valid: false, errors: ['Invalid format'], warnings: [] })
    ...
    it('should handle user cancellation', async () => {
      ...
      await expect(promptService.promptForField(sampleField, context))
  ...
  describe('promptForGroup', () => {
    ...
    it('should prompt for all fields in group', async () => {
      ...
      const result = await promptService.promptForGroup('DATABASE', groupFields, context);
  ...
  describe('shouldAutoDerive', () => {
    ...
    it('should return true when field has transformer with available source', async () => {
      ...
      const result = await promptService.shouldAutoDerive(transformerField, context);
  ...
  describe('collectUserInput', () => {
    it('should collect text input for string fields', async () => {
      ...
      const result = await promptService.collectUserInput(sampleField, 'Enter value:');
    ...
    it('should collect number input for number fields', async () => {
      ...
      const result = await promptService.collectUserInput(numberField, 'Enter port:');
    ...
    it('should provide choices for boolean fields', async () => {
      ...
      const result = await promptService.collectUserInput(booleanField, 'Enable SSL:');
    ...
    it('should handle user cancellation', async () => {
      ...
      await expect(promptService.collectUserInput(sampleField, 'Enter value:'))
  ...
  describe('processNonInteractive', () => {
    ...
    it('should process all fields with default values', async () => {
      ...
      const mockProcessNonInteractive = vi.spyOn(promptService, 'promptForField')
        .mockResolvedValueOnce({ value: 'existing-db-url', skipped: false, derived: false })
        .mockResolvedValueOnce({ value: 'default-key', skipped: false, derived: false });

      const result = await promptService.processNonInteractive(fields, defaultValues);
    ...
    it('should handle auto-derived fields', async () => {
      ...
      const result = await promptService.processNonInteractive(fieldsWithAuto, new Map());
  ...
  describe('displaySummary', () => {
    it('should display summary of results', () => {
      const results = new Map([
        ...
        ['FIELD4', { value: '', skipped: false, derived: false, error: 'Failed' }]
      ]);
